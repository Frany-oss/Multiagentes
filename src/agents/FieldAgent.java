...
public class FieldAgent extends EntityAgent {
    ...
    protected void fight() {
        if (getLocal().goal_enemy != null && status == STATES.FIGHTING) {
            ...
            //double p_e = 1.0 * getLocal().goal_enemy.getPoints() / (getLocal().getPoints() + getLocal().goal_enemy.getPoints());
            if (Math.random() < p_l) {
    ...
    protected void sendMessageAllAllies(TYPE_MESSAGES type_message, Object o) {
        ...
        switch (type_message) {
            case ENEMY: 
                mensaje.setContent(ENEMY + "_" + ((FieldUnity)o).getID() + "_" + getLocal().position.getX() + "_" + getLocal().position.getY());
