...
public abstract class Entity {
    ...
    public void setGoal(int x, int y) { setGoal(new Position(x, y)); }
    ...
    public void goToGoal() {
        if(!hasArrived()) {
            ...
            if (deltaX > 0) {
                ...
                if (signX == -1)
                    x_tmp = Math.floor(signX * speed * Math.cos(angleMovement));
                double y_tmp = Math.ceil(speed * Math.sin(angleMovement));
                if (signY == -1)
                    y_tmp = Math.floor(signY * speed * Math.sin(angleMovement));
    ...
    public boolean hasArrived() { return position.isClose(goal); }
